...
df_combined_select=df_combined[df_combined['Subfolder'].str.contains('Body')]
df_combined_select
...
def write_XML_annotation_header(Namafolder, Subfolder, Namafile, Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix):
	...
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision\\"+str(Namafolder)+str(appendix)+".xml"
...
def close_current_XML_file(Namafolder_Previous, appendix_Previous):
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision\\"+str(Namafolder_Previous)+str(appendix_Previous)+".xml"
	f = open(path_to_output_xml, "a")
	...
	print("D:\\XML_Revision\\"+str(Namafolder_Previous)+str(appendix_Previous)+".xml successfuly created")
	f.close()
