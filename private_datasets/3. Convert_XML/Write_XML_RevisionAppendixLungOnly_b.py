...
# path_to_combined_csv="D:\\CSV\\Combined_KDC6.csv"
path_to_combined_csv="D:\\CSV\\Combined_KDC6_16485.csv"
    ## Extract combined CSV header
df_combined = pd.read_csv(path_to_combined_csv)
df_combined = df_combined[['Nomor','Namafolder','Subfolder','Namafile','Xpix','Ypix','Zdown','Spacing','Size_mm','Xbound','Ybound','Zorigin','TipeNodul']]
df_combined_select=df_combined[df_combined['Subfolder'].str.contains('Lung')]
df_combined_select

total_combined_data=df_combined_select.shape[0]

Namafolder_Previous='first'
appendix_Previous='first'
...
def write_XML_annotation_header(Namafolder, Subfolder, Namafile, Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix):

	## Path to output XML
	path_to_output_xml="D:\\XML_Revision_Lung\\"+str(Namafolder)+str(appendix)+".xml"
...
def write_edgemap(Xbound, Ybound, appendix):
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision_Lung\\"+str(Namafolder)+str(appendix)+".xml"
	f = open(path_to_output_xml, "a")
...
def write_new_roi(Zorigin, Namafile, Xbound, Ybound, appendix):
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision_Lung\\"+str(Namafolder)+str(appendix)+".xml"
	f = open(path_to_output_xml, "a")
...
def write_new_nodule(Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix):
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision_Lung\\"+str(Namafolder)+str(appendix)+".xml"
	f = open(path_to_output_xml, "a")
	f.write('      </roi>\n')
...
def close_current_XML_file(Namafolder_Previous, appendix_Previous):
	## Path to output XML
	path_to_output_xml="D:\\XML_Revision_Lung\\"+str(Namafolder_Previous)+str(appendix_Previous)+".xml"
	f = open(path_to_output_xml, "a")
	...
	print("D:\\XML_Revision_Lung\\"+str(Namafolder_Previous)+str(appendix_Previous)+".xml successfuly created")
	f.close()
...
for i in range(total_combined_data):
	...
	## Create subname extra
	if 'Body' in Subfolder: 
		appendix='_a'
	else:
		if 'Lung' in Subfolder: 
			appendix='_b'	
		else:
			if 'Tissue' in Subfolder: 
				appendix='_c'	
			else:
				appendix='_d'
	...
	if Namafolder_Previous==Namafolder and appendix_Previous==appendix:
		if Nomor_Previous==Nomor:
			if Zorigin_Previous==Zorigin.copy():
				write_edgemap(Xbound,Ybound, appendix)
			else:
				write_new_roi(Zorigin, Namafile, Xbound, Ybound, appendix)
		else:
			write_new_nodule(Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix)
	else:
		if Nomor_Previous==99999:
			write_XML_annotation_header(Namafolder, Subfolder, Namafile, Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix)
		else:
			close_current_XML_file(Namafolder_Previous, appendix_Previous)
			write_XML_annotation_header(Namafolder, Subfolder, Namafile, Nomor, ClassSphericity, ClassNodul, ClassMargin, Zorigin, Xbound, Ybound, appendix)
